    ...
    if(!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)){
        ...
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
                ...
                $prevQuery = "SELECT id FROM registro_vacunas WHERE correo = '".$line[1]."'";
                $prevResult = $conn->query($prevQuery);

                if($prevResult->num_rows > 0){
                    $sql = "UPDATE registro_vacunas SET nombre_completo =?, correo = ?, dpi =?, contrasena =?, vacuna =?, dosis_puestas =?, fecha_dosis =?, centro_vacunacion = ? WHERE correo = '".$correo."'";        
                    if($stmt = mysqli_prepare($conn, $sql)){
                        // Bind variables to the prepadosis_puestas statement as parameters
                        mysqli_stmt_bind_param($stmt, "ssssssss", $param_nombre_completo, $param_correo, $param_dpi, $param_contrasena, $param_vacuna, $param_dosis_puestas, $param_fecha_dosis, $param_centro_vacunacion);
